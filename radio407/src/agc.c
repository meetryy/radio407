...
typedef struct
{
    ...
    float32_t 	ring[2 * AGC_WDSP_RB_SIZE];
    float32_t 	abs_ring[AGC_WDSP_RB_SIZE]; // abs_ring is half the size of ring
    //assign constants
...
void AudioAgc_RunAgcWdsp(int16_t blockSize, float32_t (*agcbuffer)[AUDIO_BLOCK_SIZE], const bool use_stereo )
{
    ...
    for (uint16_t i = 0; i < blockSize; i++)
    {
        ...
        float32_t vo =  log10f(agc_wdsp.inv_max_input * agc_wdsp.volts);
        if(vo > 0.0)
        {
            vo = 0.0;
        }
